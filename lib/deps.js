...
const download = require('get-down');
...
function maybeDownload(alias, url, callback) {
  const dir = util.getDependency(alias, url);
  ...
  fs.exists(dir, function(exists) {
    if (exists) {
      ...
      mkdirp(dir, function(err) {
        ...
        const dl = download(url, {dest: dir, extract: true});

        dl.on('progress', function(state) {
          if (state.retry) {
            const delay = Math.round(state.delay / 1000) + 's';
            log.info('install', 'Download failed, retrying again in ' + delay);
          } else {
            let progress = Math.floor(state.received / 1024) + 'K';
            if (state.percent) {
...
exports.updateCompiler = function(callback) {
  const compilerUrl = config.get('compiler_url');
...
exports.updateLibrary = function(callback) {
  const libraryUrl = config.get('library_url');
