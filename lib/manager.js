...
const getLibraryPath = require('./util').getLibraryPath;
const minimatches = require('./util').minimatches;
 ...
 */
const Manager = exports.Manager = function Manager(options) {
  ...
  let paths = lib.concat(main);
  if (options.closure !== false) {
    const closure = getLibraryPath();
    paths = paths.concat([
...
Manager.prototype.getDependencies = function(opt_main) {
  const main = opt_main && path.resolve(opt_main);
  const scriptsLookup = this._scripts;
  const ignoreRequires = this._ignoreRequires;
  ...
  if (!this._dependencies.hasOwnProperty(mainKey)) {
    const providesLookup = {};
    const base = [];
    const depsOnly = [];
    Object.keys(scriptsLookup).forEach(function(filepath) {
      const script = scriptsLookup[filepath];
...
Manager.prototype._startWatch = function() {
  ...
  const paths = this._lib.concat(this._main);
  const gaze = new Gaze(paths);
